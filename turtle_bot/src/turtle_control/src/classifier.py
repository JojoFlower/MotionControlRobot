...
def load_dataset(pickle_path):
    ...
    df.iloc[:, :-1] = df.iloc[:, :-1].applymap(
        lambda cell: [np.real(cell), np.imag(cell)]
    )
    ...
    print(
        "Network input dim: {}, X_train: {}, X_test: {}, y_train: {}, y_test: {}".format(
            (df.shape[1] - 1, 2),
            X_train.shape,
            X_test.shape,
            y_train.shape,
...
def evaluate_model(
    ...
    classifier.add(
        Convolution1D(
            filters=8, kernel_size=1, input_shape=input_shape, activation="relu"
        )
    )
